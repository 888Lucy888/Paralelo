...
#include <stdlib.h>
#include <string.h>
#include <Windows.h>  // Include Windows header for Sleep function and time measurement

// Credits @SergioMEV Github
...
__global__ void cell_solver(board_t* boards) {
    ...
    do {
        current_cell = boards[0].cells[cell_idx];
        ...
        for (size_t index = col_idx; index < col_idx + 9 * 9; index += 9) {
            ...
            int digit_result = cell_to_digit(boards[0].cells[index]);
            if (digit_result != 0) boards[0].cells[cell_idx] &= ~(1 << digit_result);
        }
        if (cell_to_digit(current_cell) != 0) break;
        ...
        votes = __syncthreads_count(boards[0].cells[cell_idx] != current_cell);
 ...
 */
void solve_board(board_t* cpu_boards, size_t num_boards) {
    ...
    printf("Time spent: %lu milliseconds\n", end_time - start_time);
...
// Helper function to count trailing zeros in a 16-bit integer
...
int main(int argc, char** argv) {
    ...
    solve_board(&sudoku_board, 1);  // Only one board in this case
